...
def log_add_exp(a, b): # return maximum + pes
...
class DiffusionTransformer(nn.Module):
    def __init__(
        ...
        *,
        ...
        self.register_buffer('log_cumprod_at', log_cumprod_at.float()) # 0~-11,
        self.register_buffer('log_cumprod_bt', log_cumprod_bt.float()) # [-20.0538],
        self.register_buffer('log_cumprod_ct', log_cumprod_ct.float()) # -11~0, inversion of log_cumprod_at
    ...
    def q_pred(self, log_x_start, t):           # q(xt|x0)
        ...
        # log_x_start is a standard 513 log onehot
        t = (t + (self.num_timesteps + 1))%(self.num_timesteps + 1)
        log_cumprod_at = extract(self.log_cumprod_at, t, log_x_start.shape)         # at~   log_cumprod_at shape is torch.Size([16, 1, 1]) , value varies from tensor(-0.5521) tensor(-0.5521)
        ...
        log_probs = torch.cat(
            [
                log_add_exp(log_x_start[:,:-1,:]+log_cumprod_at, log_cumprod_bt), # torch.Size([16, 512, 512])
                log_add_exp(log_x_start[:,-1:,:]+log_1_min_cumprod_ct, log_cumprod_ct) # torch.Size([16, 1, 512])
            ],
            dim=1
        )
        # print_value(log_probs, 'log_probs')  -20.0538~0

        return log_probs

    def predict_start(self, log_x_t, cond_emb, t, class_label=None):          # p(x0|xt)
        x_t = log_onehot_to_index(log_x_t) # x_t in predict_start shape is  torch.Size([16, 512]), value varies from  tensor(0) tensor(512)
        if self.amp == True:
            with autocast():
                out = self.transformer(x_t, cond_emb, class_label=class_label, t=t)
        else:
            out = self.transformer(x_t, cond_emb, class_label=class_label, t=t)
        assert out.size(0) == x_t.size(0)
        assert out.size(1) == self.num_classes-1
        assert out.size()[2:] == x_t.size()[1:]
        log_pred = F.log_softmax(out.double(), dim=1).float() # log_pred in predict_start shape is  torch.Size([16, 512, 512]), value varies from tensor(-7.5718) tensor(-2.4071)
        ...
        log_pred = torch.cat((log_pred, self.zero_vector), dim=1) # log_pred new in predict_start shape is  torch.Size([16, 513, 512]), value varies from tensor(-70.) tensor(-2.4071)
    ...
    def p_pred(self, log_x, cond_emb, t, class_label, guidew=0):             # if x0, first p(x0|xt), than sum(q(xt-1|xt,x0)*p(x0|xt))
        if self.parametrization == 'x0':
            if abs(guidew)>1e-3:
                batch_size=log_x.shape[0]
                class_label = torch.cat((class_label, torch.zeros_like(class_label)), dim=0)
                log_x_recon = self.predict_start(log_x.repeat(2,1,1), cond_emb.repeat(2,1,1) if cond_emb != None else cond_emb, t.repeat(2), class_label=class_label)[:, :-1]
                log_z1 = log_x_recon[:batch_size]
                log_z2 = log_x_recon[batch_size:]
                log_new_x_recon = (1+guidew)*log_z1 - guidew*log_z2
                # log(one hot /sum exp)
                log_new_x_recon -= torch.logsumexp(log_new_x_recon, dim=1, keepdim=True)
                log_new_x_recon = log_new_x_recon.clamp(-70, 0)
                if self.zero_vector is None or self.zero_vector.shape[0] != batch_size:
                    self.zero_vector = torch.zeros(batch_size, 1, self.content_seq_len).type_as(log_new_x_recon)- 70
                log_x_recon = torch.cat((log_new_x_recon, self.zero_vector), dim=1)
            else:
                log_x_recon = self.predict_start(log_x, cond_emb, t, class_label)
            log_model_pred = self.q_posterior(
                log_x_start=log_x_recon, log_x_t=log_x, t=t)
        elif self.parametrization == 'direct':
            log_model_pred = self.predict_start(log_x, cond_emb, t, class_label)
        else:
            raise ValueError
        return log_model_pred

    @torch.no_grad()
    def p_sample(self, log_x, cond_emb, t, prob=None, class_label=None, guidew=0):               # sample q(xt-1) for next step from  xt, actually is p(xt-1|xt)
        model_log_prob = self.p_pred(log_x, cond_emb, t, class_label, guidew=guidew)
        out = self.log_sample_categorical(model_log_prob)
        return out

    def log_sample_categorical(self, logits):           # use gumbel to sample onehot vector from log probability
        uniform = torch.rand_like(logits) #  logits shape is torch.Size([16, 513, 512]) , value varies from tensor(-49.2348) tensor(-0.0113)
        gumbel_noise = -torch.log(-torch.log(uniform + 1e-30) + 1e-30) # gumbel_noise shape is torch.Size([16, 513, 512]), value varies from tensor(-4.2352) tensor(14.4383) 这个边界由[0, 1)可推
        sample = (gumbel_noise + logits).argmax(dim=1) # sample in log_sample_categorical shape is torch.Size([16, 512]) , value varies from tensor(0) tensor(512)
    ...
    def _train_loss(self, x, cond_emb, class_label, is_train=True):                       # get the KL loss
        b, device = x.size(0), x.device

        assert self.loss_type == 'vb_stochastic'
        x_start = x   # t shape is [bs, T]
        t, pt = self.sample_time(b, device, 'importance')  # t shape is [bs]
        # self.num_classes is 513

        log_x_start = index_to_log_onehot(x_start, self.num_classes) # log_x_start shape is  torch.Size([16, 513, 512]), value varies from  tensor(-69.0776) tensor(0.)
        log_xt = self.q_sample(log_x_start=log_x_start, t=t) # log_xt shape is  torch.Size([16, 513, 512]), value varies from  tensor(-69.0776) tensor(0.)
        xt = log_onehot_to_index(log_xt) # xt shape is  torch.Size([16, 512]), value varies from  tensor(X) tensor(512)')

        ############### go to p_theta function ###############
        log_x0_recon = self.predict_start(log_xt, cond_emb, class_label=class_label, t=t)            # P_theta(x0|xt)
    ...
    def forward(
        ...
        sample_image = input.type_as(input)
        # cont_emb = self.content_emb(sample_image)

        if self.condition_emb is not None and text is not None:
            with autocast(enabled=False):
                with torch.no_grad():
                    cond_emb = self.condition_emb(text) # B x Ld x D   #256*1024
                cond_emb = cond_emb.float()
        elif self.condition_emb is not None and img is not None:
            cond_emb = self.condition_emb(img)
        ...
        if is_train == True:
            log_model_prob, loss = self._train_loss(sample_image, cond_emb, class_label)
    ...
    def sample(
        ...
        if prob != None:
            zero_vector = torch.zeros(1, prob.shape[1], prob.shape[-1]).type_as(prob)- 70
            prob = torch.cat((prob, zero_vector), dim=0)
        ...
        if start_step == 0:
            ...
            with torch.no_grad():
                for diffusion_index in range(start_step-1, -1, -1):
                    ...
                    log_z = self.p_sample(log_z, cond_emb, t, prob, class_label=class_label, guidew=guidew)
