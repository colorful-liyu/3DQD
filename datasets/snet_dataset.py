...
class ShapeNetDataset(BaseDataset):
    ...
    def initialize(self, opt, phase='train', cat='all'):
        ...
        for c in all_cats:
            ...
            with open(f'{dataroot}/ShapeNet/filelists/{synset}_{phase}.lst') as f:
                ...
                self.class_list += [i] * len(model_list_s)
                i += 1
        ...
        np.random.default_rng(seed=0).shuffle(self.class_list)
        ...
        self.class_list = self.class_list[:self.max_dataset_size]
    ...
    def __getitem__(self, index):
        ...
        # sdf_h5_file = osp.join(self._data_dir, 'SDF_v1', synset, model_id, 'ori_sample.h5')
    ...
    def initialize(self, opt, phase='train', cat='all'):
        ...
        for c in all_cats:
            ...
            with open(f'{dataroot}/ShapeNet/filelists/{synset}_{phase}.lst') as f:
                ...
                self.class_list += [i] * len(model_list_s[:nimgs_to_take])
                i += 1
                print('[*] %d samples for %s (%s).' % (len(model_list_s), self.id_to_cat[synset], synset))
...
class ShapeNetMultiCodeDataset(BaseDataset):
    ...
    def initialize(self, opt, phase='train', cat='all'):
        ...
        code_setting = get_code_setting(opt)
        self.code_dir = f'{dataroot}/extracted_code/{code_setting}'
        self.coarse_code_dir = f'{dataroot}/multiscale_temp'
        assert os.path.exists(self.code_dir), f'{self.code_dir} should exist.'
        assert os.path.exists(self.coarse_code_dir), f'{self.coarse_code_dir} should exist.'
        ...
        for c in all_cats:
            ...
            with open(f'{dataroot}/ShapeNet/filelists/{synset}_{phase}.lst') as f:
                ...
                for l in f.readlines():
                    ...
                    # path = f'{dataroot}/ShapeNet/SDF_v1_64/{synset}/{model_id}/ori_sample.h5'
                    path = f'{self.code_dir}/{synset}/{model_id}'
                    coarse_path = f'{self.coarse_code_dir}/{synset}/{model_id}'
                    model_list_s.append(path)
                    coarse_model_list_s.append(coarse_path)

                    # sdf_p = f'{path}/x.npy'
                    # code_p = f'{path}/code.npy'
                    # codeix_p = f'{path}/codeix.npy'
                    # coarse_codeix_p = f'{coarse_path}/codeix.npy'
                    # self.sdf.append(torch.from_numpy(np.load(sdf_p)))
                    # self.code.append(torch.from_numpy(np.load(code_p)))
                    # self.codeix.append(torch.from_numpy(np.load(codeix_p)))
                    # self.coarse_codeix.append(torch.from_numpy(np.load(coarse_codeix_p)))
                ...
                # self.cats_list += [synset] * len(model_list_s)
                self.model_list += model_list_s[:nimgs_to_take]
                self.coarse_model_list += coarse_model_list_s[:nimgs_to_take]
                self.cats_list += [synset] * len(model_list_s[:nimgs_to_take])
                self.class_list += [i] * len(model_list_s[:nimgs_to_take])
                i += 1
                print('[*] %d samples for %s (%s).' % (len(model_list_s), self.id_to_cat[synset], synset))
        ...
        np.random.default_rng(seed=0).shuffle(self.model_list)
        np.random.default_rng(seed=0).shuffle(self.coarse_model_list)
        np.random.default_rng(seed=0).shuffle(self.cats_list)
        np.random.default_rng(seed=0).shuffle(self.class_list)
        # np.random.default_rng(seed=0).shuffle(self.sdf)
        # np.random.default_rng(seed=0).shuffle(self.code)
        # np.random.default_rng(seed=0).shuffle(self.codeix)
        # np.random.default_rng(seed=0).shuffle(self.coarse_codeix)
        # assert len(self.sdf) == len(self.code) == len(self.codeix) == len(self.coarse_codeix) == len(self.model_list)

        # cprint('[*] (SDFDataset) there are %d categories.' % (len(all_catids)), 'yellow')

        # need to check the seed for reproducibility
        self.model_list = self.model_list[:self.max_dataset_size]
        self.coarse_model_list = self.coarse_model_list[:self.max_dataset_size]
        self.class_list = self.class_list[:self.max_dataset_size]
        # self.sdf = self.sdf[:self.max_dataset_size]
        # self.code = self.code[:self.max_dataset_size]
        # self.codeix = self.codeix[:self.max_dataset_size]
        # self.coarse_codeix = self.coarse_codeix[:self.max_dataset_size]
        cprint('[*] %d samples loaded.' % (len(self.model_list)), 'yellow')
    ...
    def __getitem__(self, index):
        ...
        class_label = self.class_list[index]
        model = self.model_list[index]
        coarse_model = self.coarse_model_list[index]

        sdf_p = f'{model}/x.npy'
        code_p = f'{model}/code.npy'
        codeix_p = f'{model}/codeix.npy'
        coarse_codeix_p = f'{coarse_model}/codeix.npy'

        sdf = torch.from_numpy(np.load(sdf_p))
        code = torch.from_numpy(np.load(code_p))
        codeix = torch.from_numpy(np.load(codeix_p))
        coarse_codeix = torch.from_numpy(np.load(coarse_codeix_p))

        # sdf = self.sdf[index]
        # code = self.code[index]
        # codeix = self.codeix[index]
        # coarse_codeix = self.coarse_codeix[index]

        ret = {
            'sdf': sdf,
            'z_q': code,
            'idx': codeix,
            'coarse_idx': coarse_codeix,
            'cat_id': synset,
            'class_label': class_label,
            'cat_str': self.id_to_cat[synset],
            'path': model,
    ...
    def name(self):
        return 'ShapeNetMultiCodeDataset'
